...
def postprocess(results: dict[str, list], conditions: dict[str, Any]) -> dict[str, list]:
    ...
    print("Started values of gain", len(gain_mc_arr))

    # Delete statistical outliers in gain_mc_arr COMMENTED TO MATCH ARRAY LENGTH
    ...
    print("Started values of bw", len(bw_mc_arr))

    # Delete statistical outliers in bw_mc_arr COMMENTED TO MATCH ARRAY LENGHT
    # bw_mc_arr = [val for val in bw_mc_arr if 10e3 <= val <= 10e9]
    # print(f'bw_mc_arr = {bw_mc_arr}')
    
    print(len(bw_mc_arr), len(gain_mc_arr))
    # Save data as .csv for later use
    np.savetxt('cace/scripts/foldedcascode-ac.csv', 
        np.column_stack((np.array(gain_mc_arr), np.array(bw_mc_arr))), comments = "", header = "gain_mc_arr,bw_mc_arr", delimiter = ",")
