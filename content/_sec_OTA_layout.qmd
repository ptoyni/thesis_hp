...
Mathematically, the effective property $P$ (e.g., threshold voltage or current factor) for device A can be expressed as:
...
When the interdigitated pattern is symmetric, the mean values $\overline{P_A}$ and $\overline{P_B}$ become equal, effectively cancelling first-order spatial variation.
...
In mathematical terms, if the gradient is expressed as 

$$
P(x, y) = P_0 + a x + b y,
$$

then the centroid placement ensures that

$$
\sum_i (a x_i + b y_i) = 0
$$

for each matched device, thereby cancelling first-order gradient terms.
