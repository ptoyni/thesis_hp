...
Folded Cascode OTA (see@_fig_foldedcascode_diagram.svg) is very much aligned with @wicht2020 methodology, especially if our focus is on:
...
As shown in the @_fig_foldedcascode_diagram.svg, $M_1$ and $M_2$ are the input differential pair mosfets with $M_5,_6$ and M_7,_8 as their cascoded mosfets.
...
|:-------------------------------------------|:------------:|:-------:|
| Output voltage error                       | $<1.2$         | %       |
| Total output noise (rms)                   | $<0.2$       | mV~rms~ |
