...
The AES campus is intended to create an advanced, collaborative environment that aims to identify gaps along the chip design process chain with open source tools. Using the example of the development of an LED driver IC, these are closed and the maturity of the tools for practical application is improved in order to advance and establish mixed signal chip design with open source tools.

The main goal is to extend the open source toolchain of Prof. Pretl's IIC-OSIC-TOOLS with the IHP OpenPDK SG13G2 in such a way that in the first step AES GmbH and in the second step all SMEs can also use this toolchain for future product design and experience. To verify this, an exemplary LED driver is being designed with HSB and AES GmbH.
